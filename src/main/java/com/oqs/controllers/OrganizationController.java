...
public class OrganizationController {
    ...
    public List<User.SimpleUser> organizationsBySort(@RequestParam("typeId") String typeId) {
        ...
        List<User.SimpleUser> simpleUsers = new ArrayList<User.SimpleUser>();
        for(User user: organizationList){
            simpleUsers.add(user.getSmplUser(user));
        }
        return simpleUsers;
    }
    ...
    public String changePhoto(@PathVariable("organizationId") long organizationId, @RequestParam("files") MultipartFile file) {
        fileUpload.fileUpload(file, organizationId);
        User user = userDAO.get(organizationId);
        user.setPhoto("/resources/img/organizations/bsn-" + organizationId + ".jpg");
        userDAO.update(user);
