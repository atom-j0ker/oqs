...
function validateRole() {
    ...
    if (role != undefined)
        ...
        document.getElementById("invalid-role").innerHTML = "please, choose role";
    return false;
}
...
function validateType() {
    ...
    if (role == 'ROLE_USER') {
        ...
        var type = $('select[name=typeComboBoxName]').val()
        if(type == null)
...
function validateForm() {
    if (validateEmail() && validatePassword() && validateRole() && validateType())
        return true;
    else
        return false;
}